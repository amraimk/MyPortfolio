...
describe('Login', () => {
    ...
    test('sends the credentials to the API and stores the token', async () => {
        ...
        fireEvent.change(screen.getByPlaceholderText(/email/i), { target: { name: 'email', value: 'student@test.com' } });
        fireEvent.change(screen.getByPlaceholderText(/password/i), { target: { name: 'password', value: '123456' } });
        ...
        expect(window.localStorage.setItem).toHaveBeenCalledWith('name', 'st123');
        expect(window.localStorage.setItem).toHaveBeenCalledWith('role', 'student');
